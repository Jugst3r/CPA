...
unsigned *access_matrix(int *adja_matrix, unsigned int i, unsigned int j, unsigned int nb_nodes){
  return adja_matrix+(nb_nodes*i)+j;
}
...
unsigned *adjacency_matrix(FILE *f_in, int *rename_tab, unsigned int nb_nodes){
    

  unsigned *adja_matrix = (unsigned *)malloc(sizeof(unsigned)*nb_nodes*nb_nodes);
  if(adja_matrix == NULL){
...
int main(int argc, char *argv[]) {
  ...
  clock_t start = clock();
  if(argc < 3){
    printf("./tme3 sourcefile op\n");
    printf("Operations:\n0 for adjacency matrix\n1 for adjacency list\n2 for adjacency array\n3 for biggest component\n4 for diameter\n5 for triangle number\n");
    return 0;
  }
